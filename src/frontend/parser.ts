...
import { ArrayLiteral, FuncDeclaration, MemberExpr, StringLiteral } from "./ast";
...
export default class Parser {
    ...
    private parse_func_declaration(): Stmt {
        ...
        for (const arg of args) {
            if (arg.kind !== "Identifier")
                throw `${arg}\nInside function declaration expected parameters to be type string`;

            params.push((arg as Identifier).symbol);
        }

        this.expect(TokenType.OpenBrace, "Expected function body following declaraction");
        const body: Stmt[] = [];
        ...
        const fn = { body, name, parameters: params, kind: "FuncDeclaration", } as FuncDeclaration;
    ...
    private parse_var_declaration(): Stmt {
        ...
        const identifier = this.expect(TokenType.Identifier, "Expected identifer name following let | const keywords.").value;
    ...
    private parse_object_expr(): Expr {
        ...
        while (this.not_eof() && this,this.at().type != TokenType.CloseBrace) {
            ...
            if (this.at().type != TokenType.CloseBrace) {
                this.expect(TokenType.Comma, "Expected comma of closing bracket following property.");
            }
        }
    ...
    private parse_call_expr(caller: Expr): Expr {
        let call_expr: Expr = { kind: "CallExpr", caller, args: this.parse_args(), } as CallExpr;
    ...
    private parse_args(): Expr[] {
        this.expect(TokenType.OpenParen, "Expected open parenthises");
        const args = this.at().type == TokenType.CloseParen ? [] : this.parse_arguments_list();

        this.expect(TokenType.CloseParen, "Missing closed parenthises inside arguments list");
        return args;
    }
    ...
    private parse_primary_expr(): Expr {
        ...
        switch (tk) {
            ...
            case TokenType.OpenParen: {
                ...
                this.expect(TokenType.CloseParen, "Unexpected token found inside parenthisesed expression. Expected closing parenthises."); // eat close paren
                return value;
            }
